...
class VectorQuantizerEMA(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, commitment_cost, decay, epsilon=1e-5,beta=0.01):
    ...
    def count_code(self, inputs):
        ...
        code_dist = distances.min(1).values.reshape(seqlen,bs) 
    ...
    def reinit(self, code_encoded):
        ...
        self.count = np.zeros(self._num_embeddings).astype(int) 
    ...
    def forward(self, inputs):
        ...
        normalized_distances = F.softmax(distances, dim=1)
        ...
        average_distances = torch.mean(normalized_distances, dim=0)

        entropy = -(average_distances * torch.log(average_distances + 1e-8)).sum()
        ...
        loss = self._commitment_cost * e_latent_loss + self._beta * entropy
