...
class SolidData(torch.utils.data.Dataset):
    ...
    def __init__(self, path):  
        ...
        for data in dataset:
            ...
            if len(cuboids)<=MAX_SOLID:
                op = cuboids[:, 0] 
                num_box = len(cuboids) 
                xyz_min, xyz_max = self.get_corners(cuboids[:, 1:]) 
                vec_data = {}
                vec_data['cmd'] = op
    ...
    def get_corners(self, boxes):
        ...
        xyz_min = np.concatenate([x_min[:,np.newaxis], y_min[:,np.newaxis], z_min[:,np.newaxis]], 1) 
        x_span, y_span, z_span = x_max-x_min, y_max-y_min, z_max-z_min
        xyz_span = np.concatenate([x_span[:,np.newaxis], y_span[:,np.newaxis], z_span[:,np.newaxis]], 1) 
