...
def sample(args):
    ...
    for data_sample, data_uid in load_iter:
        if(len(data_uid)>0):
            countnum+=1
            xy_sample_temp = sample_cad[data_uid[0]][0]
            ext_sample_temp = sample_cad[data_uid[0]][1]
            uid_temp = hash_sketch(xy_sample_temp, ext_sample_temp)
            if uid_temp not in pair_uid:
                pair_uid[uid_temp]=[countnum-1]
            else:
                pair_uid[uid_temp].append(countnum-1)
    ...
    if args.mode == 'eval':
        ...
        for sample in sample_cad:
            if(len(raster_cad_out(sample))>0): 
                xy_sample = sample[0]
                ext_sample = sample[1]
                uid = hash_sketch(xy_sample, ext_sample)
                if uid not in duplicate_groups:
        ...
        gen_count = 0
        non_unique_count = 0
        non_valid_count=len(sample_cad)-countnum
        for key, value in duplicate_groups.items():
            ...
            if 2 in value and 1 in value:
                ...
                if(key in pair_uid):
                    no_novel_pair[key]=pair_uid[key]
            if 2 in value:
                non_unique_count += (np.array(value)==2).sum()-1
                if(key in pair_uid and (np.array(value)==2).sum()==1):
                    unique_pair[key]=pair_uid[key]
                if(key in pair_uid and (np.array(value)==2).sum()>1):
                    no_unique_pair[key]=pair_uid[key]
                if(key in pair_uid and 1 not in value):
                    novel_pair[key]=pair_uid[key]
        summary_data = {
            'NovelNumber': [countnum - non_novel_count],
            'UniqueNumber': [countnum - non_unique_count],
        }

        save_data_to_txt(args.coutpair_file, summary_data)
        ...
        save_data_to_txt(args.novel_pair_file,novel_pair)
        save_data_to_txt(args.unique_pair_file,unique_pair)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--novel_pair_file", type=str, required=True)
    parser.add_argument("--unique_pair_file", type=str, required=True)
    parser.add_argument("--coutpair_file", type=str, required=True)
